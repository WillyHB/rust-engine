...
use game_engine::{
    animation::{
        animation_from_spritesheet, {Animation, AnimationParams},
    },
    ecs::components::{
        animated_sprite::AnimatedSprite, collider::Collider, sprite::Sprite, velocity::Velocity,
    },
    input::keyboard,
};
...
impl Player {
    pub async fn new(speed: f32) -> Player {
        ...
        let walk_anim =
            animation_from_spritesheet("walk", player_texture.clone(), 16, 20, 5, 8, 12, &params)
                .await;
        let idle_anim =
            animation_from_spritesheet("idle", player_texture.clone(), 16, 20, 1, 4, 6, &params)
                .await;
        let run_anim =
            animation_from_spritesheet("run", player_texture.clone(), 16, 20, 4, 8, 12, &params)
                .await;

        params.repeat = false;
        let fall_anim =
            animation_from_spritesheet("fall", player_texture.clone(), 16, 20, 6, 3, 6, &params)
                .await;
        let land_anim =
            animation_from_spritesheet("land", player_texture.clone(), 16, 20, 3, 6, 12, &params)
                .await;
        let jump_anim =
            animation_from_spritesheet("jump", player_texture.clone(), 16, 20, 2, 4, 12, &params)
                .await;
    ...
    pub fn is_grounded(
        ...
        for col in &mut collider_query.into_iter() {
            if "ground".to_string() == col.tag && player_collider.collides(col.tag.clone()) {
                return true;
            }
        }
...
pub fn player_update(
    mut query: Query<(&mut Velocity, &Collider, &mut AnimatedSprite, &mut Player)>,
    collider_query: Query<&Collider, Without<Player>>,
) {
    for (mut velocity, collider, mut sprite, mut player) in &mut query {
        ...
        if !player.is_grounded(collider, &collider_query) {
            if !player.is_jumping {
                sprite
                    .animator
                    .play_once(Animation::from(&player.fall_anim));
            }
            continue;
        }
