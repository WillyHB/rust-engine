...
use macroquad::texture::{
    draw_texture, draw_texture_ex, load_texture, render_target, set_default_filter_mode,
    DrawTextureParams, RenderTarget, Texture2D,
};
...
async fn main() -> Result<(), String> {
    ...
    for vel in velocities2.get_mut().iter_mut().flatten() {
    ...
    let _ground = world
        .spawn((
            Position {
                vec: Vec2f32::new(0.0, 300.0),
            },
            Collider::new(Rect::new(0.0, 0.0, 1000.0, 50.0), String::from("ground")),
        ))
        .id();

    let _left_wall = world
        .spawn((
            Position {
                vec: Vec2f32::new(-50.0, 0.0),
            },
    ...
    'running: loop {
        ...
        if keyboard::is_pressed(KeyCode::Q) {
            break 'running;
        }
