...
pub fn render_sprites(
    mut query: Query<(&SpriteRenderer, &Sprite, &Position), Without<AnimatedSprite>>,
) {
    for (renderer, sprite, position) in &mut query {
        ...
        params.dest_size = Some(Vec2::new(
            sprite.destination_bounds.w,
            sprite.destination_bounds.h,
        ));
        ...
        draw_texture_ex(
            &sprite.texture,
            position.vec.x as f32,
            position.vec.y as f32,
            sprite.colour,
            params,
        );
    }
...
pub fn render_animated_sprites(
    mut query: Query<(&SpriteRenderer, &mut AnimatedSprite, &Position), Without<Sprite>>,
) {
    for (renderer, anim_sprite, position) in &mut query {
        if !anim_sprite.animator.has_animation() {
            return;
        }
        ...
        params.dest_size = Some(Vec2::new(50.0, 50.0));
