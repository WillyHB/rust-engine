...
pub fn apply_collision(
    mut actor_query: Query<(&Position, &mut Velocity, &mut Collider)>,
    ...
    for (actor_pos, mut actor_velocity, mut actor_collider) in &mut actor_query {
        for (solid_position, solid_collider) in &mut solids_query {
            let predicted_rect_x = actor_collider.global_bounds(Vec2f32::new(
                actor_pos.vec.x + actor_velocity.vec.x,
                actor_pos.vec.y,
            ));
            let predicted_rect_y = actor_collider.global_bounds(Vec2f32::new(
                actor_pos.vec.x,
                actor_pos.vec.y + actor_velocity.vec.y,
            ));

            let solid_rect = solid_collider
                .global_bounds(Vec2f32::new(solid_position.vec.x, solid_position.vec.y));
